...
def nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth):
    ...
    month = 1
    p_c = (startPriceOld-startPriceNew)*(1-percentLossByMonth/100)
    bank = p_c+savingperMonth
    if bank>=0:
        return [month,round(bank)]
    month+=1
    while (bank<0):
        if month%2==0:
            percentLossByMonth+=0.5
        p_c *=(1-percentLossByMonth/100)
        bank=p_c+savingperMonth*month
        month+=1
    return [month-1,round(bank)]