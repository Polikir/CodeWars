...
def bowling_score(frames):
    ...
    summ = 0
    for i in range(9):
        frames[i] = norm(frames[i])
    if len(frames[9]) == 2:
        frames[9] = norm(frames[9])
    else:
        if frames[9][1] == '/':
            if frames[9][2] != 'X':
                frames[9] = [int(frames[9][0]), 10 - int(frames[9][0]), int(frames[9][2])]
            else:
                frames[9] = [int(frames[9][0]), 10 - int(frames[9][0]), 10]
        else:
            if frames[9][2] == 'X':
                frames[9] = [10, 10, 10]
            else:
                frames[9] = [10, 10, int(frames[9][2])]
    for i in range(9):
        if frames[i][0] == 10:
            if frames[i+1][0] == 10:
                if i != 8:
                    summ += 10 + 10 + frames[i+2][0]
                else:
                    summ += 20 + frames[9][1]
            else:
                summ += (10 + frames[i + 1][0] + frames[i+1][1])
        elif sum(frames[i]) == 10:
            summ += (10 + frames[i + 1][0])
        else:
            summ += sum(frames[i])
    summ += sum(frames[9])
    return summ